...
def compute_base_nodes_from_hexes(
    hexes: List[Tuple[int, int, int, int, int, int, int, int]],
    slice_to_eids: Dict[int, List[int]],
) -> List[int]:
    """
    Reproduce old logic: BASE = outer radial faces (nodes 4..7) of the
    first existing radial slice, based on the hexa mesh.
    """
    existing_slice_idxs = sorted(
        i for i, eids in slice_to_eids.items() if eids
    )
    if not existing_slice_idxs:
        return []
    ...
    for eid in base_eids:
        ...
        # cylindrical case: outer radial face = nodes 4..7
        base_node_set.update([n4, n5, n6, n7])

    return sorted(base_node_set)


def hex_layers_to_tets_with_tetgen(
    ...
    For each radial slice:
      - Take all hexa elements in that slice.
      - Build its boundary surface (remove internal faces).
      - Triangulate quads -> triangles.
      - Call TetGen on that PLC to generate a tetrahedral mesh.

    We maintain a *global* vertex list. TetGen preserves boundary vertices;
    we re-use global node IDs by hashing coordinates with a tolerance.
    New interior TetGen points get appended to the global vertex list.

    Returns
    -------
    new_vertices : list[(x,y,z)]
        Global node list (original + TetGen interior nodes).
    tets : list[(n1,n2,n3,n4)]
        4-node C3D4-style tetrahedra (1-based node IDs).
    slice_to_tet_eids : dict[slice_idx, list[int]]
        Mapping slice -> list of tetra element IDs.
    """
    # Start with existing vertices as global list
    new_vertices: List[Tuple[float, float, float]] = list(vertices)

    # Coordinate -> global node id map, using quantized coords
    coord_to_gid: Dict[Tuple[int, int, int], int] = {}

    def key_from_coord(p: Tuple[float, float, float]) -> Tuple[int, int, int]:
        x, y, z = p
        return (
            int(round(x / coord_tol)),
            int(round(y / coord_tol)),
            int(round(z / coord_tol)),
        )

    for gid, (x, y, z) in enumerate(new_vertices, start=1):
        coord_to_gid[key_from_coord((x, y, z))] = gid

    tets: List[Tuple[int, int, int, int]] = []
    slice_to_tet_eids: Dict[int, List[int]] = {k: [] for k in slice_to_eids.keys()}

    # Face patterns for a hex (node order as in your connectivity)
    face_patterns = [
        (0, 1, 2, 3),  # bottom
        (4, 5, 6, 7),  # top
        (0, 1, 5, 4),  # side
    ...
    for slice_idx, hex_eids in sorted(slice_to_eids.items()):
        if not hex_eids:
            continue

        # 1) Collect boundary faces of this slice
        faces_map: Dict[Tuple[int, int, int, int], int] = {}
        # use key = sorted nodes to identify duplicates (internal faces)
        tmp_face_store: Dict[Tuple[int, int, int, int], Tuple[int, int, int, int]] = {}

        for eid in hex_eids:
            n = hexes[eid - 1]  # (n0..n7)
            for pattern in face_patterns:
                face = tuple(n[i] for i in pattern)  # original order
                key = tuple(sorted(face))
                if key not in faces_map:
                    faces_map[key] = 1
                    tmp_face_store[key] = face
                else:
                    faces_map[key] += 1

        # boundary faces = faces that appear only once in this slice
        boundary_faces: List[Tuple[int, int, int, int]] = []
        for key, count in faces_map.items():
            if count == 1:
                boundary_faces.append(tmp_face_store[key])

        if not boundary_faces:
            log(f"[TETGEN] Slice {slice_idx}: no boundary faces, skipping.")
            continue

        # 2) Boundary nodes of this slice (global IDs)
        boundary_node_ids: List[int] = sorted(
            {nid for face in boundary_faces for nid in face}
        )

        # Local index mapping (TetGen nodes are 0..N-1)
        global_to_local: Dict[int, int] = {
            gid: i for i, gid in enumerate(boundary_node_ids)
        }
        local_points = np.array(
            [vertices[gid - 1] for gid in boundary_node_ids],
            dtype=float,
        )

        # 3) Triangulate boundary quads into triangles
        tri_list: List[Tuple[int, int, int]] = []
        for (a, b, c, d) in boundary_faces:
            la = global_to_local[a]
            lb = global_to_local[b]
            lc = global_to_local[c]
            ld = global_to_local[d]
            # two triangles: (a,b,c) and (a,c,d)
            tri_list.append((la, lb, lc))
            tri_list.append((la, lc, ld))

        triangles = np.array(tri_list, dtype=int)

        log(
            f"[TETGEN] Slice {slice_idx}: "
            f"{len(boundary_node_ids)} boundary nodes, "
            f"{len(boundary_faces)} quads -> {len(triangles)} triangles"
        )

        # 4) Call TetGen on this slice's PLC
        # PyVista tetgen wrapper: tetrahedralize() returns (nodes, elems, attr, triface_markers)
        t = tetgen.TetGen(local_points, triangles)

        # steinerleft=-1 = allow unlimited Steiner points (you already like this setting)
        nodes, elems, *_ = t.tetrahedralize(order=1, steinerleft=-1)

        # nodes: (Nt, 3), elems: (Ne, 4) or (Ne, 10)
        tet_points = np.array(nodes, dtype=float)
        tet_connect = np.array(elems, dtype=int)
        # if you ever want to support 10-node tets, you’d just use the first 4 cols here
        # tet_connect = np.array(elems[:, :4], dtype=int)

        # 5) Map TetGen points to global node IDs using coord hash
        local_index_to_global: Dict[int, int] = {}
        for local_idx, p in enumerate(tet_points):
            p_tuple = (float(p[0]), float(p[1]), float(p[2]))
            k = key_from_coord(p_tuple)
            gid = coord_to_gid.get(k)
            if gid is None:
                gid = len(new_vertices) + 1
                new_vertices.append(p_tuple)
                coord_to_gid[k] = gid
            local_index_to_global[local_idx] = gid

        # 6) Build global tet connectivity and slice mapping
        for row in tet_connect:
            # TetGen usually uses 0-based indices
            n0 = local_index_to_global[int(row[0])]
            n1 = local_index_to_global[int(row[1])]
            n2 = local_index_to_global[int(row[2])]
            n3 = local_index_to_global[int(row[3])]
            tets.append((n0, n1, n2, n3))
            eid_global = len(tets)
            slice_to_tet_eids[slice_idx].append(eid_global)

        log(
            f"[TETGEN] Slice {slice_idx}: generated {tet_connect.shape[0]} tets, "
            f"global vertices now {len(new_vertices)}"
        )

    return new_vertices, tets, slice_to_tet_eids
...
def write_calculix_job_tet(
    ...
    """
    Same additive-style uncoupled temperature-displacement job as
    write_calculix_job(), but for C3D4 tetra elements.

    base_nodes: node IDs to be fixed as BASE (outer radial face, taken
    from the original hexa mesh).
    """
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write("**\n** Auto-generated incremental-cure shrink job (tet)\n**\n")
        f.write("*HEADING\n")
        f.write(
            "Voxel C3D4 uncoupled temperature-displacement "
        ...
        # -------------------- BASE --------------------
        if base_nodes:
            f.write("** Base node set (from original hexa mesh) +++++++++++++++\n")
            f.write("*NSET, NSET=BASE\n")
            _write_id_list_lines(f, sorted(base_nodes))
        else:
            f.write("** Warning: no BASE node set provided.\n")
        ...
        if n_slices == 0 or not slice_names:
            ...
            existing_slice_idxs = [
                int(name.split("_")[1]) for name in slice_names
            ]
    ...
    log(f"[CCX] (tet) Wrote incremental-cure UT-D job to: {path}")
    log(
        f"[CCX] (tet) Nodes: {n_nodes}, elements: {n_elems}, "
...
def main():
    ...
    # --- Build tetrahedral layers from voxelized hexa layers ---
    base_nodes = compute_base_nodes_from_hexes(hexes, slice_to_eids)

    tet_vertices, tet_elements, slice_to_tet_eids = hex_layers_to_tets_with_tetgen(
        vertices,
        hexes,
        slice_to_eids,
    )

    # 2) Single uncoupled thermo-mechanical job (tet-based)
